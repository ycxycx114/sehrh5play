...
(function ($) {
    ...
    var DanmuPlayer = function (element, options) {
        ...
        this.$element.append('<video class="danmu-video" x5-video-player-type="h5" preload="metadata" playsinline="true" webkit-playsinline="true"  x-webkit-airplay="true"x5-video-orientation="portraint" x5-video-player-fullscreen="true" src="' + options.src + '" width="' + options.width + '" height="' + options.height + '"></video>');
        ...
        this.$ctrlMain.append('<div class="full-screen   ctrl-btn-left"><span class="" aria-hidden="true">全屏Full Screen</span></div>');
        ...
        $(this.id + " .full-screen").on("click", {video: this.video, that: that}, function (e) {
            if (!e.data.that.danmuPlayerFullScreen) {
				...
				$(e.data.that.id + " .full-screen span").html('退出全屏Normal Screen');
            }
            else {
                $(e.data.that.id).removeClass("danmu-player-full-screen");
                e.data.that.danmuPlayerFullScreen = false;
				$(e.data.that.id + " .full-screen span").html('全屏Full Screen');
            }
